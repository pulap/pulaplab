...
(defn default-header []
  [:ul {:class (styles/get-class :menu-list)}
   [:li [:a {:href "/private/auth/list-users" :class (styles/get-class :header-link)} "Users"]]
   [:li [:a {:href "/private/auth/list-roles" :class (styles/get-class :header-link)} "Roles"]]
   [:li [:a {:href "/private/auth/list-permissions" :class (styles/get-class :header-link)} "Permissions"]]
   [:li [:a {:href "/private/auth/list-resources" :class (styles/get-class :header-link)} "Resources"]]
   [:li [:a {:href "/res/todo" :class (styles/get-class :header-link)} "Todo"]]])
       ...
       (for [{:keys [id username email]} users]
         [:tr {:key id}
          ...
          [:td {:class (styles/get-class :td-actions)}
           ...
           [:form {:action "/private/auth/delete-user" :method "POST" :class "inline"}
            [:input {:type "hidden" :name "id" :value id}]
            [:button {:type "submit"
                      :class (styles/get-class :button-delete)} "Delete"]]]])]]
     [:div {:class "flex justify-center mt-6"}
      [:a {:href "/private/auth/new-user"
           :class (styles/get-class :button-new)} "New"]]]
   ...
   {:title "New User"
    ...
    [:div {:class "flex justify-center"}
     [:div {:class "bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full max-w-md"}
      ...
      [:form {:action "/private/auth/create-user" :method "POST" :class "space-y-6"}
       [:input {:type "hidden" :name "id" :value ""}]
       [:div
        [:label {:for "username" :class (styles/get-class :form-label)} "Username"]
        [:input {:type "text" :name "username" :id "username"
                 :required true
                 :class (styles/get-class :form-input)}]]
       [:div
        [:label {:for "email" :class (styles/get-class :form-label)} "Email"]
       ...
       [:div
        ...
        [:input {:type "text" :name "name" :id "name"
                 :class (styles/get-class :form-input)}]]
       [:div
        [:label {:for "password" :class (styles/get-class :form-label)} "Password"]
        [:input {:type "password" :name "password" :id "password"
                 :required true
                 :class (styles/get-class :form-input)}]]
       [:div
        [:label {:for "password_conf" :class (styles/get-class :form-label)} "Confirm Password"]
